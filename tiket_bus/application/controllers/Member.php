...
*/
	...
	public function jadwal()
	{
		...
		foreach ($data['bus'] as $key => $value) {
			
			$_Ebus[$key] = $this->db->query('select * from pemesanan_tiket where idbus = ? and idjadwal = ?', array($value->idd,$value->idj))->result();

			

			foreach ($_Ebus[$key] as $keys => $values) {
				
				if($values->jumlahtiket > 0){

					@$jumlahtiket[$values->idbus][$values->idjadwal] += $values->jumlahtiket;

					$data['sisa_tiket'][$values->idbus][$values->idjadwal] = $value->jumlahtiket - @$jumlahtiket[$values->idbus][$values->idjadwal];

				}

			}

			#$data['jumlahtiket'][$value->id] = $values->jumlahtiket - $jumlahtiket;


		}

		//getting sum jumlahtiket
	...
	//pemesanan tiket
		...
		{	
			...
			$query['data'] = $this->m_login->pemesanantiket_print($id,$iduser);
			#$this->pre($query);
			$this->load->view('member/print_bukti',$query, FALSE);
		}

		public function pemesanantiket($id,$idj)
		{
			$iduser = $this->session->userdata('id');
			$data = array(
				'nama' => $this->session->userdata('nama'),
				'log' => $this->db->query("select * from login where id = ?",array($iduser))->row(),
				'stat' => $this->session->userdata('stat'),
				'bus' => $this->m_login->getbus($id,$idj),
				'sheet' => $this->m_login->getsheet($id),
				'tiket_pesanan' => $this->db->query('SELECT * FROM `pemesanan_tiket`, bus, jadwal where bus.id = pemesanan_tiket.idbus and jadwal.id = pemesanan_tiket.idjadwal and pemesanan_tiket.idbus = ? and pemesanan_tiket.idjadwal = ?',array($id,$idj))->result(),
				'get_shet' => $this->db->query('select * from jenis_sheet, bus where bus.idshet = jenis_sheet.id and bus.id = ?',array($id))->row(),
			);

			//proses updating when tgl_upload terakhir reach that limit and stat is Tolak or Waiting for response admin

			//get ordered ticket
			if(count($data['tiket_pesanan']) > 0){
				...
				$arr_imp[] = implode(',', $shet); //implode be one phrase
				...
				//proses decided true or false each sheet
		...
		function ins_tiket(){
			...
			#print_r($db);
			#$_Qbus = $this->db->query('select * from bus where id = ?',$db['idbus'])->row();
			#$jumlah_tiket = $_Qbus->jumlahtiket - $db['jumlahtiket'];

			#$update = $this->db->query('update bus set jumlahtiket = ? where id = ?',array($jumlah_tiket, $db['idbus']));
		...
		function up_tiket($id){
			...
			if ( ! $this->upload->do_upload('bukti')){
				$error = array('error' => $this->upload->display_errors());
				echo '<Script>alert("pilih format yang benar!!")</script>';
				#echo '<script>alert("Format File harus jpg/jpeg !! Max Width 1024, max Weight 768");</script>';
