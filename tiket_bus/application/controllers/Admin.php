...
class Admin extends CI_Controller {
	...
	var $pemesanantiket = 'SELECT *, login.nama as nama_user, hari.nama as namahari, jamberangkat.jam as jamberangkat, bus.bus as namabus, ntujuan.nama as namatujuan, pemesanan_tiket.jumlahtiket as jumtiket, pemesanan_tiket.status as namastat, pemesanan_tiket.id as idpesan, pemesanan_tiket.id as idtiket
		FROM `pemesanan_tiket`, login, jadwal, hari, jamberangkat, bus, ntujuan
		where 
		pemesanan_tiket.iduser = login.id and 
		jadwal.id = pemesanan_tiket.idjadwal and 
		jamberangkat.id = jadwal.berangkat and 
		jadwal.hari = hari.id and
		bus.id = jadwal.idbus and
		jadwal.tujuan = ntujuan.id';
	...
	function esop($id){
		$data = array(
			'nama' => $this->session->userdata('nama'),
			'sopir' => $this->db->get_where('sopir',array('id' => $id))->row(),
		);
		#var_dump($data);
...
/*
	...
	public function konfirmasi()
	{
		$data = array(
			...
			'tgl_now' => $this->db->query('select now() as now')->row(),
			#tab1
			#'tabel1' => $this->db->query($this->tab1)->result(),
			#tab2
			'tabel2' => $this->db->query($this->pemesanantiket)->result(),
		);
		#var_dump($data['query']);
		$date = date('D');
		//getting value of day
		

		$datetime = $this->db->query('select now() as now')->row();

		foreach ($data['tabel2'] as $key => $value) {
			
		

			if($datetime->now > $value->tgl_expired){



				#$update = $this->db->query('update bus set jumlahtiket = ? where id = ?',array($jumlahtiket,$value->idbus));
				$delete =  $this->db->query('delete from pemesanan_tiket where id = ?',array($value->idtiket));
		...
		//proses updating when tgl_upload terakhir reach that limit and more then that day go
	...
	function agree($id,$data,$table){
		...
		$query = $this->db->query('update '.$table.' set status = ? where id = ?',array($data,$id));
		#$this->pre($query);
		if($query == true){
	...
	function disband($id,$data,$table){

		$query = $this->db->query('update '.$table.' set status = ? where id = ?',array($data,$id));
		#$this->pre($query);
		if($query == true){

			echo '<script>alert("Berhasil Tolak")</script>';
			redirect('admin/konfirmasi','refresh');
	...
	public function laporan_tiket()
	{
		...
		$inputan_f = $this->input->post('tgl_awal').' 00:00:00';
		$inputan_l = $this->input->post('tgl_akhir').' 23:59:00';
			...
			$data = array(
				...
				'data' => $this->db->query('select * from laporan_tiket where tgl_expired between ? and ?',array($inputan_f,$inputan_l))->result(),
				);
		...
		#$this->load->vars($data);
		$this->load->view('admin/Header', $data, FALSE);
		$this->load->view('admin/Laporan_tiket');
		$this->load->view('admin/Footer');

	}

	function cetak_laporan($tabel, $awal, $akhir){

		// $data = array(
		// 	'tabel' => $tabel,
		// 	'awal' => $awal.' 00:00:00',
		// 	'akhir' => $akhir. '23:59:00',
		// 	);
		...
		if($tabel == 'laporan_tiket'){
			...
			$query['data'] =  $this->db->query('select * from laporan_tiket where tgl_expired between ? and ?',array($awal, $akhir))->result();
			$query['tabel'] = $tabel;
			$query['awal'] = $awal;
			$query['akhir'] = $akhir;
			$this->load->view('admin/cetak_laporan', $query, FALSE);
		}else{

			$query['data'] =  $this->db->query('select * from npemesanan_pariwisata where tgl_expied between ? and ?',array($awal, $akhir))->result();
			$query['tabel'] = $tabel;
			$query['awal'] = $awal;
			$query['akhir'] = $akhir;
			$this->load->view('admin/cetak_laporan_pariwisata', $query, FALSE);
	...
	//start laporan pariwisata

	public function laporan_pariwisata()
	{
		$inputan = $this->input->post();
		$inputan_f = $this->input->post('tgl_awal').' 00:00:00';
		$inputan_l = $this->input->post('tgl_akhir').' 23:59:00';
			...
			$data = array(
				...
				'tgl_awal' => $this->input->post('tgl_awal'),
				'tgl_akhir' => $this->input->post('tgl_akhir'),
				'data' => $this->db->query('select * from npemesanan_pariwisata where tgl_expied between ? and ?',array($inputan_f,$inputan_l))->result(),
			);
		...
		#$this->load->vars($data);
		$this->load->view('admin/Header', $data, FALSE);
		$this->load->view('admin/laporan_pariwisata');
		$this->load->view('admin/Footer');

	}

	//end laporan pariwisata
