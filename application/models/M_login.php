...
class M_login extends CI_Model {
	...
	function getbus($id){
		return $this->db->query('SELECT *, jadwal.id as idj ,bus.id as idd, ntujuan.nama as naju FROM `jadwal`, bus, ntujuan, jamberangkat,hari, jenis_sheet where jadwal.idbus = bus.id and jadwal.tujuan = ntujuan.id and jadwal.berangkat = jamberangkat.id and jadwal.hari = hari.id and jenis_sheet.id = bus.idshet and bus.id = ?',array($id))->row();
	}
	...
	function konfirmasipemesanantiket($id){
		return $this->db->query('select *, pemesanan_tiket.id as idtiket, pemesanan_tiket.jumlahtiket as juket, ntujuan.nama as naju, pemesanan_tiket.status as natus from pemesanan_tiket, login, jadwal, ntujuan, bus where pemesanan_tiket.iduser = login.id and pemesanan_tiket.idjadwal = jadwal.id and jadwal.tujuan = ntujuan.id and bus.id = jadwal.idbus and pemesanan_tiket.iduser = ? order by pemesanan_tiket.id desc',array($id))->row();
	}

	function showpemesanantiket($id){
		return $this->db->query('select *, hari.nama as namahari, jamberangkat.jam as jamberangkat, pemesanan_tiket.jumlahtiket as juket, ntujuan.nama as naju, pemesanan_tiket.status as natus, pemesanan_tiket.id as idtiket from pemesanan_tiket, login, jadwal, ntujuan, bus, hari, jamberangkat where pemesanan_tiket.iduser = login.id and pemesanan_tiket.idjadwal = jadwal.id and jadwal.tujuan = ntujuan.id and bus.id = jadwal.idbus and hari.id = jadwal.hari and jamberangkat.id = jadwal.berangkat and login.id = ?',array($id))->result();
	}

	function showuppemesanantiket($iduser,$id){
		return $this->db->query('select *, pemesanan_tiket.jumlahtiket as juket, ntujuan.nama as naju, pemesanan_tiket.status as natus, pemesanan_tiket.id as idtiket from pemesanan_tiket, login, jadwal, ntujuan, bus where pemesanan_tiket.iduser = login.id and pemesanan_tiket.idjadwal = jadwal.id and jadwal.tujuan = ntujuan.id and bus.id = jadwal.idbus and login.id = ? and pemesanan_tiket.id = ?',array($iduser,$id))->row();
	}
